...
# globals -----
  ...
  exp_clinic$variables <- 
    globals$clin_vars[!globals$clin_vars %in% c('neoadjuvant')]

  ## analysis tables
  
  exp_clinic$analysis_tbl <- list()
  
  for(i in globals$cohort_lexicon$cohort) {
    
    eval(parse_expr(paste0('exp_clinic$analysis_tbl$', i, 
                           ' <- ', i, '$expression')))
    
    
  }
  
  exp_clinic$analysis_tbl[c('tcga', 'gse167093')] <- 
    exp_clinic$analysis_tbl[c('tcga', 'gse167093')] %>% 
    map(filter, tissue_type == 'Tumor')
  
  exp_clinic$analysis_tbl <- exp_clinic$analysis_tbl %>% 
    map(~select(.x,
                any_of(exp_clinic$variables),
...
# Testing ----
  ...
  exp_clinic$top_factors <- exp_clinic$test_results %>% 
  ...
  exp_clinic$plots <- list(cohort = set_names(names(exp_clinic$variables),
    ...
    future_pmap(function(cohort, suffix) set_names(globals$cxcl_genes,
           ...
           map(function(gene) list(split_factor = exp_clinic$variables[[cohort]],
                                   x_lab = exchange(exp_clinic$variables[[cohort]], 
...
# Summary plots: effect sizes and significance for tumor, node and meta stages ------
  ...
  exp_clinic$etest_stages <- exp_clinic$etest_stages %>%
    ...
    exchange('cohort', 
             ...
             key = 'cohort', 
             value = 'label') %>% 
    mutate(variable = paste(variable, cohort, sep = ', '))
