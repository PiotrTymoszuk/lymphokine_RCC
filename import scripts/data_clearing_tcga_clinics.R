...
# reading the relevant clinical data ------
  ...
  tcga_clinics <- 
    c(drugs = './data/TCGA/nationwidechildrens.org_clinical_drug_kirc.txt',
      fup = './data/TCGA/nationwidechildrens.org_clinical_follow_up_v1.0_kirc.txt',
      clinical = './data/TCGA/nationwidechildrens.org_clinical_patient_kirc.txt',
      radiation = './data/TCGA/nationwidechildrens.org_clinical_radiation_kirc.txt') %>%
    map(read_tsv)
  ...
  tcga_clinics$clinical <- tcga_clinics$clinical[c(-1, -2), ] %>%
    transmute(patient_id = bcr_patient_barcode,
           histology = factor(car::recode(histologic_diagnosis,
                                          "'[Discrepancy]' = NA")),
           tumor_grade = factor(car::recode(tumor_grade, 
                                            "'[Not Available]' = NA")),
