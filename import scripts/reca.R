...
# reading the clinical, sample, specimen and expression data -------
  ...
  reca[c('clinical',
         'specimen',
         'sample')] <- c('./data/RECA/donor.RECA-EU.tsv',
                         './data/RECA/specimen.RECA-EU.tsv',
                         './data/RECA/sample.RECA-EU.tsv') %>%
    map(read_tsv)

  reca$expression <- read_tsv('./data/RECA/exp_seq.RECA-EU.tsv')
...
# clearing the clinical, sample and specimen data ------
  ...
  reca$clinical <- reca$clinical %>%
    ...
    transmute(patient_id = icgc_donor_id,
              ...
              pt_stage = stri_replace(donor_tumour_stage_at_diagnosis, 
                                      regex = 'N.*$', 
                                      replacement = ''),
              pt_stage = ifelse(is.na(pt_stage) | 
                                  stri_detect(pt_stage, fixed = 'T'),
                                pt_stage, paste0('T', pt_stage)),
              pt_stage = factor(pt_stage),
              pn_stage = stri_extract(donor_tumour_stage_at_diagnosis, 
                                      regex = 'N.{1}'),
              pn_stage = factor(pn_stage),
              pm_stage = stri_extract(donor_tumour_stage_at_diagnosis, 
                                      regex = 'M.{1}'),
...
# clearing the expression data ---------
  ...
  reca$expression <- 
    reca$expression[c('icgc_specimen_id', 'gene_id', 'normalized_read_count')] %>%
