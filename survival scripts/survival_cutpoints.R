  ...
  surv_cut$plot_titles <- names(surv_cut$analysis_tbl) %>%
    map(~list(stri_split_fixed(.x,
              ...
              stri_split_fixed(.x,
                               pattern = '_',
                               simplify = TRUE)[, 2])) %>%
    map_chr(~paste(globals$cohort_lexicon$label[.x[[1]]],
                   toupper(.x[[2]]),
                   sep = ', '))
...
# stratification ------
  ...
  surv_cut[paste0(globals$cxcl_genes, '_cutoff')] <- 
    globals$cxcl_genes %>%
...
# significance plots -------
  ...
  surv_cut$summ_plot_titles <- globals$cxcl_genes %>% 
  ...
  surv_cut$summ_plots <-
    list(data = surv_cut$test_results,
         plot_title = surv_cut$summ_plot_titles) %>%
...
# Kaplan-Meier plots -----
  ...
  surv_cut[paste0(globals$cxcl_genes, '_plots')] <- 
    surv_cut[paste0(globals$cxcl_genes, '_cutoff')] %>%
  ...
  surv_cut[paste0(globals$cxcl_genes, '_plots')] <- 
    list(x = surv_cut[paste0(globals$cxcl_genes, '_plots')],
