...
# analysis globals -------
  ...
  multi_rfs$genes <- globals$cxcl_genes
  
  ## analysis tables
  ## adding a day to OS, because the algorithm does not tolerate
  ## immediate censoring - no effects on modeling results expected
  
  multi_rfs$analysis_tbl <- list()
  
  for(i in c('tcga', 'cm10', 'cm25ev', 'cm25ni')) {
    
    eval(parse_expr(paste0('multi_rfs$analysis_tbl$', i, 
                           ' <- ', i, '$expression')))
    
    
  }
  
  multi_rfs$analysis_tbl <- multi_rfs$analysis_tbl %>% 
    map(select, 
        patient_id, 
        rfs_days,
        relapse, 
        any_of('tissue_type'), 
        all_of(multi_rfs$genes)) %>% 
    map(~filter(.x, complete.cases(.x)))
  
  multi_rfs$analysis_tbl$tcga <- 
    multi_rfs$analysis_tbl$tcga %>% 
  ...
  multi_rfs$y <- multi_rfs$analysis_tbl %>% 
    map(~Surv(.x$rfs_days, .x$relapse))
  
  multi_rfs$x <- multi_rfs$analysis_tbl %>% 
    map(column_to_rownames, 'patient_id') %>% 
    map(~.x[multi_rfs$genes]) %>% 
  ...
  multi_rfs$folds <- 1:multi_rfs$n_rep %>% 
    map(~createFolds(y = multi_rfs$analysis_tbl$tcga$patient_id, 
                     k = 10, 
                     list = FALSE, 
                     returnTrain = TRUE)) %>% 
    set_names(paste0('rep_', 1:multi_rfs$n_rep))
...
# lambda tuning -------
  ...
  multi_rfs$lambda_tune <- multi_rfs$folds %>% 
    future_map(~cv.glmnet(x = multi_rfs$x$tcga, 
                          y = multi_rfs$y$tcga, 
                          foldid = .x, 
                          type.measure = 'default', 
                          family = 'cox', 
                          alpha = multi_rfs$alpha), 
               .options = furrr_options(seed = TRUE))
  
  ## extracting the optimal lambdas
  
  multi_rfs$lambda_tbl <- multi_rfs$lambda_tune %>% 
    map(~as_tibble(.x[c('lambda', 'cvm', 'cvup', 'cvlo', 
                        'lambda.min', 'lambda.1se')])) %>% 
    map_dfr(filter, lambda == lambda.1se)
  
  multi_rfs$opt_lambda <- multi_rfs$lambda_tbl %>% 
    filter(cvm == min(cvm))
...
# Building the GLMNET model -------
  ...
  multi_rfs$glmnet_model <- glmnet(x = multi_rfs$x$tcga, 
                                   y = multi_rfs$y$tcga, 
                                   family = 'cox', 
                                   alpha = multi_rfs$alpha, 
                                   lambda = multi_rfs$opt_lambda$lambda)
  
# Linear predictor scores ------
  
  insert_msg('Linear predictor scores')
  
  multi_rfs$lp_scores <- multi_rfs$x %>% 
    map(~predict(multi_rfs$glmnet_model, 
                 newx = .x, 
                 type = 'link')) %>% 
    map(as.data.frame) %>% 
    ...
    map(as_tibble) %>% 
    map2(., multi_rfs$analysis_tbl, 
         left_join, by = 'patient_id')
  
  ## stratification of the lp score in tertiles 
  
  multi_rfs$lp_scores <- multi_rfs$lp_scores %>%
    map(~mutate(.x, 
...
# Uni variate Cox models of the LP score ------
  ...
  multi_rfs$cox_models <- multi_rfs$lp_scores %>% 
    map(~coxph(Surv(rfs_days, relapse) ~ lp_score, 
               data = .x)) %>% 
    map2(., multi_rfs$analysis_tbl, 
         as_coxex)
...
# Assumptions ------
  ...
  multi_rfs$assumptions <- multi_rfs$cox_models %>% 
...
# Fit stats -------
  ...
  multi_rfs$fit_stats <- multi_rfs$cox_models %>% 
    map(summary, 'fit') %>%
...
# Non-zero model coefficients ------
  ...
  multi_rfs$coefs <- multi_rfs$glmnet_model %>% 
    coef %>% 
    as.matrix %>% 
    as.data.frame %>% 
...
# Coefficient plot ---------
  ...
  multi_rfs$coef_plot <- multi_rfs$coefs %>% 
    ggplot(aes(x = log(hr), 
...
# LP score tertile KM plots --------- 
  ...
  multi_rfs$surv_fit <- multi_rfs$lp_scores %>% 
    map(~surv_fit(Surv(rfs_days, relapse) ~ lp_strata, 
                  data = .x))
  
  ## log-rank test
  
  multi_rfs$surv_diff <- multi_rfs$surv_fit %>% 
    map(surv_pvalue, 
        method = 'survdiff', 
        test.for.trend = TRUE) %>% 
    map(as_tibble) %>% 
  ...
  multi_rfs$surv_strata_n <- multi_rfs$lp_scores %>% 
    map(count, lp_strata) %>% 
  ...
  multi_rfs$km_plots <- 
    list(fit = multi_rfs$surv_fit, 
         title = globals$cohort_lexicon$label[names(multi_rfs$surv_fit)], 
         legend.labs = multi_rfs$surv_strata_n, 
         pval = multi_rfs$surv_diff$plot_lab) %>% 
    pmap(ggsurvplot, 
  ...
  multi_rfs$km_plots <- 
    list(x = multi_rfs$km_plots, 
         y = multi_rfs$fit_stats$plot_cap, 
         z = multi_rfs$fit_stats$plot_tag) %>% 
