...
Clinical and normalized RNAseq expression data for the RECA-EU cohort were fetched from https://dcc.icgc.org/projects/RECA-EU. 
...
For the microarray expression studies, integration of multiple probes targeting the same gene was accomplished by geometric mean. The clinical characteristic of the analyzed studies is provided in __Table 1__.
...
The chemokine genes of interest (_CXCL9/10/11_, _CXCL13_ and _XCR1_) were identified to be preferentially expressed in the RCC tissue as compared with the normal kidney and correlate with predicted tumor-infiltrating CD8^+^ T cell levels as described in more details below. 
...
Genes associated with interferon alpha/beta signaling and interferon gamma signaling were obtained from the Reactome database (pathways: R-HSA-909733 and R-HSA-877300). 

## Software

The statistical and bioinformatic analysis was done with R 4.2.0. 
Tabular data were handled with the _tidyverse_ package bundle [@Wickham2019] and the development package _trafo_ (https://github.com/PiotrTymoszuk/trafo). 
Text data were transformed with the _stringi_ package [@Gagolewski2021]. 
Prediction of non-malignant cell content was done with the _immunedeconv_ package [@Sturm2020]. 
Gene signatures were retrieved, stored and their scores calculated with the _gseaTools_ development package (https://github.com/PiotrTymoszuk/gseaTools). 
Parallelization was accomplished with _furrr_ [@Vaughan2022].

UMAP (Uniform Manifold Approximation and Projection) clustering [@McInnes2018; @Konopka2022] and semi-supervised clustering was done with the development package _clustTools_ (https://github.com/PiotrTymoszuk/clustTools) providing a common interface to multiple dimensionality reduction and clustering algorithms and employing among others algorithms from the packages _cluster_ [@Schubert2019], _philentropy_ [@Drost2018], _factoextra_ [@Kassambara2020], _dbscan_ [@Hahsler2019] and _umap_ [@Konopka2022]. 
...
For survival analysis, the packages _survival_ [@Therneau2000], _survminer_ [@Kassambara2016], _glmnet_ [@Friedman2010] and the development package _coxExtensions_ (https://github.com/PiotrTymoszuk/coxExtensions) were employed. 
Differential gene expression analysis and comparison of Reactome ssGSEA score between the clusters was done with the development package _microViz_ (https://github.com/PiotrTymoszuk/microViz). 
Modulation of signaling pathways was investigated with the _SPIA_ algorithm [@Tarca2009]. 
Analysis of differences in metabolic reaction activity was done with the _Biggr_ [@Gavai2015] package and the development package _biggrExtra_ (https://github.com/PiotrTymoszuk/biggrExtra).
...
Management of figures and tables in the Rmarkdown documents (insertion, referencing) was done with the development package _figur_ (https://github.com/PiotrTymoszuk/figur).
...
In addition, median-centered normalized datasets of xCell infiltration estimates were subjected to two-dimensional UMAP (with cosine distance between the observations, function `reduce_data(red_fun = 'umap', kdim = 2)`, package _clustTools_ employing tools provided by _umap_) [@McInnes2018; @Konopka2022]. 
Normalized expression levels of the genes of interest and normalized levels of specific immune cell types were drawn in UMAP layout plots (method `plot()`, package _clustTools_) to visualize clusters of cancer samples characterized by high chemokine expression and extensive infiltration by T cells, myeloid dendritic cells (mDC) and tumor-associated macrophages (TAM).
...
```{r tab-spia, tab.cap = 'Genes differentially regulated between the chemokine clusters (chemokine high vs low). The table is available as a supplementary Excel file.'}
...
__Supplementary Figure S\@ref(fig:fig-clust-infil). Predicted infiltration of T cells, activated mDC and TAM in the chemokine clusters, CheckMate cohorts.__ 
...
_Differences in distribution of tumor stages (A) and MSKCC risk strata (B) between the chemokine (chemox) high and low clusters cancer samples were assessed by false discovery rate (FDR) corrected $\chi^2$ test and Cramer V effect size statistic. Effect sizes and p values are displayed in the plot captions, numbers of cancer samples assigned to the chemokine clusters are indicated in the plot x axes. CM 010: CheckMate 010; CM 025 EVER: CheckMate 025 everolimus; CM 025 NIVO: CheckMate 025 nivolumab._
...
```{r fig-os1, fig.width = mm_inch(suppl_figures$os1$w), fig.height = mm_inch(suppl_figures$os1$h), fig.cap = 'Modeling of overall survival as a function of cancer expression of CXCL9/10/11 and CXCR3.'}

suppl_figures$os1$plot

```

__Supplementary Figure S\@ref(fig:fig-os1). Modeling of overall survival as a function of cancer expression of *CXCL9/10/11* and *CXCR3*.__ 

_Correlation of log~2~-transformed gene expression with survival was investigated with a series of uni-variable Cox regression models. Linear and spline terms of gene expression were included in the models. Hazard ratio (HR) estimates for the linear gene expression terms with 95% confidence intervals (CI) are presented in Forest plots. Point color codes for significance and sign of correlation with survival. Total numbers of patients and deaths are indicated in the Y axes. CM 010: CheckMate 010; CM 025 EVER: CheckMate 025 everolimus; CM 025 NIVO: CheckMate 025 nivolumab._

\newpage

```{r fig-os2, fig.width = mm_inch(suppl_figures$os2$w), fig.height = mm_inch(suppl_figures$os2$h), fig.cap = 'Modeling of overall survival as a function of cancer expression of CXCL13, CXCR5, XCL1 and XCR1.'}

suppl_figures$os2$plot

```

__Supplementary Figure S\@ref(fig:fig-os2). Modeling of overall survival as a function of cancer expression of *CXCL13*, *CXCR5*, *XCL1* and *XCR1*.__ 

_Correlation of log~2~-transformed gene expression with survival was investigated with a series of uni-variable Cox regression models. Linear and spline terms of gene expression were included in the models. Hazard ratio (HR) estimates for the linear gene expression terms with 95% confidence intervals (CI) are presented in Forest plots. Point color codes for significance and sign of correlation with survival. Total numbers of patients and deaths are indicated in the Y axes. CM 010: CheckMate 010; CM 025 EVER: CheckMate 025 everolimus; CM 025 NIVO: CheckMate 025 nivolumab._

\newpage

```{r fig-rfs1, fig.width = mm_inch(suppl_figures$rfs1$w), fig.height = mm_inch(suppl_figures$rfs1$h), fig.cap = 'Modeling of relapse-free survival as a function of cancer expression of CXCL9/10/11 and CXCR3.'}

suppl_figures$rfs1$plot

```

__Supplementary Figure S\@ref(fig:fig-rfs1). Modeling of relapse-free survival as a function of cancer expression of *CXCL9/10/11* and *CXCR3*.__ 

_Correlation of log~2~-transformed gene expression with survival was investigated with a series of uni-variable Cox regression models. Linear and spline terms of gene expression were included in the models. Hazard ratio (HR) estimates for the linear gene expression terms with 95% confidence intervals (CI) are presented in Forest plots. Point color codes for significance and sign of correlation with survival. Total numbers of patients and relapses are indicated in the Y axes. CM 010: CheckMate 010; CM 025 EVER: CheckMate 025 everolimus; CM 025 NIVO: CheckMate 025 nivolumab._

\newpage

```{r fig-rfs2, fig.width = mm_inch(suppl_figures$rfs2$w), fig.height = mm_inch(suppl_figures$rfs2$h), fig.cap = 'Modeling of relapse-free survival as a function of cancer expression of CXCL13, CXCR5, XCL1 and XCR1.'}

suppl_figures$rfs2$plot

```

__Supplementary Figure S\@ref(fig:fig-rfs2). Modeling of relapse-free survival as a function of cancer expression of *CXCL13*, *CXCR5*, *XCL1* and *XCR1*.__ 
